...
def makemuudatused(i, forshow, kuupaev):
    if kuupaev == True:
        ...
        elif len(i) > 5 and (i[5].receivedMsgText() == "" or i[5].receivedMsgText() == " "):
            forshow.append(f"🗓 {i[0]} Дата: {i[1]}\n🦆 Группа: {i[2]} ⏰ Урок: {i[3]}\n👨‍🏫 Преподаватель: {i[4]}\n")
        else:
...
for event in longpoll.listen():
    ...
    if event.type == VkEventType.MESSAGE_NEW:
        if event.to_me:
            ...
            if receivedMsgText == "начать" or receivedMsgText == 'start':
            ...
            elif receivedMsgText == "указать группу" or receivedMsgText == "изменить группу":
                usergroup = openfromfile(usergroup)
                write_msg(event.user_id, event.random_id,
